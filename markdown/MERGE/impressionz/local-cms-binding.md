...
### ⚠️ IMPORTANT: Cross-Collection References

**CMS data may reference data in other CMS databases/collections.** Understanding these relationships is crucial when binding data.

**Common Patterns:**
- **One-to-Many:** Service → Service Offerings (one service has many offerings)
- **Many-to-Many:** Services ↔ Stylists (services reference stylists, stylists reference services)
- **Reference Fields:** Items link to other collection items via slug

When binding data, always:
1. Identify all collections that are referenced
2. Define all collections needed for the page
3. Look up related items using slug references
4. Resolve relationships before rendering

### Organize Data by Type

Keep your data organized in separate arrays for each collection:
...
// Service Offerings collection (referenced by Services)
const offeringsData = [
  { name: "Offering 1", slug: "offering-1", serviceLine: "service-1", ... }
];

// Stylists collection (referenced by Services)
const stylistsData = [
  { name: "Team Member 1", slug: "stylist-1", ... }
];
```

### Use Consistent Slugs for Relationships

Link related items using slug arrays. **CMS data references other CMS collections by slug:**

```javascript
{
  name: "Hair Services",
  slug: "hair-services",
  // Cross-collection references - these reference items in other collections
  serviceOfferings: ["haircuts", "color", "styling"], // Array of slugs from Offerings collection
  stylists: ["stylist-1", "stylist-2"] // Array of slugs from Stylists collection
}
```

Then look them up from other collections:
...
function bindServiceDetail(serviceSlug) {
  ...
  // Look up related items from Offerings collection
  if (service.serviceOfferings && service.serviceOfferings.length > 0) {
    service.serviceOfferings.forEach(offeringSlug => {
      ...
      if (offering) {
        // Render offering (this item is from a different collection)
      }
    });
  }
  
  // Look up related items from Stylists collection
  if (service.stylists && service.stylists.length > 0) {
    service.stylists.forEach(stylistSlug => {
      ...
      if (stylist) {
        // Render stylist (this item is from a different collection)
      }
    });
  }
}
...
**Key Point:** CMS data often references data in other CMS databases/collections. Always check for these relationships when analyzing CMS structure and binding data.
